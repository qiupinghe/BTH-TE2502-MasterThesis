...
{
  std::unique_ptr<Botan::StreamCipher> cipher(Botan::StreamCipher::create("CTR(AES-128)"));
  ...
  std::ofstream ctr_iv("ctr_iv", std::ios::binary | std::ios::ate);
  ...
  std::ofstream ctr_output("ctr_enc.bin", std::ios::binary | std::ios::ate);
  while(file.good())
  {
